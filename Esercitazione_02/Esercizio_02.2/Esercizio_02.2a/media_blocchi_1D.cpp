...
void Blocchi(int L, int N, double mean[], double square[]){
   ...
   int i,j,l; //Contatori.
   int a; //Variabile estratta o 0 o 1.
   int r; //Variabile da integrare. E' la posizione finale.   
   int sum; //Somma degli elementi in un blocco. 
   ...
   for(i=0; i<N; i++){ //Per ogni blocco
      ...
      for(j=0; j<L; j++){ //All'interno di ciascun blocco.
	     r = 0; //Azzero i passi ad ogni ciclo.		 
	  
         for(l=0; l<100; l++){ //Estrae un numero decimale in [0,1), lo moltiplica per 2
            a = int(2.0*rnd.Rannyu()); //e ne prende la parte intera: si ottiene così o 0 o 1.
	       
	        if(a == 1){ //Se hai estratto 1, incrementa la posizione di 1.
		       r++;
	        }
	        else{ //Se hai estratto 0, torna indietro di 1.
		       r--;
   	        }	  	  
         }  
		 /*A 1 associo un passo in avanti, a 0 un passo indietro.
		   Sommo i passi in avanti e indietro: è la posizione finale r, la variabile da integrare.*/ 
		 sum += r; //Somma per fare la media a blocchi.
      }	  
	  
      mean[i] = double(sum)/double(L); //Questo e' il valor medio del blocco i-esimo. 
